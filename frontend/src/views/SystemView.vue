      ...
      <el-container>
        ...
        <el-main style="background: none;">
          ...
          <div v-if="activeTab === 'role'">
            ...
            <div class="toolbar">
              <div>
                <el-button type="primary" size="small" @click="showRoleDialog('add')">
                  <el-icon><Plus /></el-icon>新增
                </el-button>
                <el-button type="success" size="small" @click="handleRoleToolbarEdit">
                  <el-icon><Edit /></el-icon>修改
                </el-button>
                <el-button type="danger" size="small" :disabled="!roleSelection.length" @click="handleRoleDelete()">
                  <el-icon><Delete /></el-icon>删除
                </el-button>
              ...
              <el-button size="small" @click="handleRoleRefresh">
                <el-icon><Refresh /></el-icon>刷新
              </el-button>
            </div>
            <el-table :data="roleTable" border style="width: 100%" @selection-change="roleSelection = $event">
              ...
              <el-table-column label="操作" width="220">
                <template #default="scope">
                  ...
                  <el-button type="danger" plain link size="small" @click="handleRoleDelete([scope.row])">
                    <el-icon style="color:#f56c6c"><Delete /></el-icon> 删除
                  </el-button>
            ...
            <el-dialog v-model="roleDialog.visible" :title="roleDialog.mode === 'add' ? '新增角色' : '编辑角色'" width="400px">
              <el-form :model="roleDialog.form" label-width="80px" :rules="roleRules" ref="roleFormRef">
                ...
                <el-form-item label="数据权限" prop="dataScope">
                  ...
                  <template v-if="roleDialog.form.dataScope === 'CUSTOM'">
                    <el-button size="small" @click="openCustomDeptDialog" style="margin-left: 16px;">选择部门</el-button>
                    <span v-if="roleDialog.form.deptNames && roleDialog.form.deptNames.length" style="margin-left: 8px; color: #888;">{{ roleDialog.form.deptNames.join('，') }}</span>
                  </template>
                </el-form-item>
                <el-form-item label="状态" prop="status">
            ...
            <!-- 角色管理分配用户弹窗 -->
            <el-dialog v-model="assignUserDialog.visible" title="分配用户" width="800px">
              <div style="margin-bottom: 10px;">
                <span>原始分配用户：</span>
                <el-tag v-for="uid in assignUserDialog.originUserIds" :key="uid" style="margin-right: 4px;">
                  {{ (allUsers.find(u => u.userId === uid) || {}).username || uid }}
                </el-tag>
                <el-button size="small" style="margin-left: 10px;" @click="resetAssignUserSelection">重置</el-button>
                <el-button size="small" style="margin-left: 4px;" @click="clearAssignUserSelection">清空选择</el-button>
                <el-button size="small" style="margin-left: 4px;" @click="clearAssignUserSearch">清空搜索框</el-button>
              </div>
              <el-input v-model="assignUserDialog.search" placeholder="请输入用户名/昵称/手机号搜索" clearable style="margin-bottom: 10px;" />
            ...
            <!-- 数据权限弹窗 -->
            <el-dialog v-model="dataScopeDialog.visible" title="修改数据权限" width="500px">
              <el-form :model="dataScopeDialog.form" label-width="100px">
                <el-form-item label="数据权限">
                  ...
                  <template v-if="dataScopeDialog.form.dataScope === 'CUSTOM'">
                    <el-button size="small" @click="openCustomDeptDialogDS" style="margin-left: 16px;">选择部门</el-button>
                    <span v-if="dataScopeDialog.form.deptNames && dataScopeDialog.form.deptNames.length" style="margin-left: 8px; color: #888;">{{ dataScopeDialog.form.deptNames.join('，') }}</span>
                  </template>
                </el-form-item>
              </el-form>
              <template #footer>
                <el-button @click="dataScopeDialog.visible = false">取消</el-button>
                <el-button type="primary" @click="handleDataScopeDialogOk">保存</el-button>
              </template>
            </el-dialog>
          </div>
          <!-- 菜单管理 -->
          <div v-if="activeTab === 'menu'">
            <el-form :inline="true" :model="menuQuery" class="search-form" size="small">
              ...
              <el-form-item>
                <el-button type="primary" @click="handleMenuSearch">
                  <el-icon><Search /></el-icon>搜索
                </el-button>
                <el-button @click="handleMenuReset">
                  <el-icon><Refresh /></el-icon>重置
                </el-button>
            ...
            <div class="toolbar">
              <div>
                <el-button type="primary" size="small" @click="showMenuDialog('add')">
                  <el-icon><Plus /></el-icon>新增
                </el-button>
                <el-button type="success" size="small" :disabled="!menuSelection" @click="showMenuDialog('edit')">
                  <el-icon><Edit /></el-icon>修改
                </el-button>
                <el-button type="danger" size="small" :disabled="!menuSelection" @click="handleMenuDelete()">
                  <el-icon><Delete /></el-icon>删除
                </el-button>
              </div>
              <el-button size="small" @click="handleMenuRefresh">
                <el-icon><Refresh /></el-icon>刷新
              </el-button>
            </div>
            <el-table
            ...
            >
              ...
              <el-table-column label="操作" width="220">
                <template #default="scope">
                  ...
                  <el-button type="danger" plain link size="small" @click="handleMenuDelete([scope.row])">
                    <el-icon style="color:#f56c6c"><Delete /></el-icon> 删除
                  </el-button>
                </template>
              </el-table-column>
          ...
          <div v-if="activeTab === 'dept'">
            <el-form :inline="true" :model="deptQuery" class="search-form" size="small">
              ...
              <el-form-item>
                ...
                <el-button @click="handleDeptReset">
                  <el-icon><Refresh /></el-icon>重置
                </el-button>
            ...
            <div class="toolbar">
              <div>
                <el-button type="primary" size="small" @click="showDeptDialog('add')">
                  <el-icon><Plus /></el-icon>新增
                </el-button>
                <el-button type="success" size="small" :disabled="!deptSelection" @click="showDeptDialog('edit')">
                  <el-icon><Edit /></el-icon>修改
                </el-button>
                <el-button type="danger" size="small" :disabled="!deptSelection" @click="handleDeptDelete()">
                  <el-icon><Delete /></el-icon>删除
                </el-button>
              </div>
              <el-button size="small" @click="handleDeptRefresh">
                <el-icon><Refresh /></el-icon>刷新
              </el-button>
            </div>
            <el-table
            ...
            >
              ...
              <el-table-column label="操作" width="220">
                <template #default="scope">
                  ...
                  <el-button type="danger" plain link size="small" @click="handleDeptDelete([scope.row])">
                    <el-icon style="color:#f56c6c"><Delete /></el-icon> 删除
                  </el-button>
                </template>
              </el-table-column>
          ...
          <div v-if="activeTab === 'config'">
            ...
            <div class="toolbar">
              <div>
                ...
                <el-button type="danger" size="small" :disabled="!configSelection.length" @click="handleConfigDelete()">
                  <el-icon><Delete /></el-icon>删除
                </el-button>
                <el-button size="small">导出</el-button>
                <el-button size="small">刷新缓存</el-button>
              </div>
              <el-button size="small" @click="handleConfigRefresh">
                <el-icon><Refresh /></el-icon>刷新
              </el-button>
            </div>
            <el-table :data="configTable" border style="width: 100%" @selection-change="configSelection = $event">
              ...
              <el-table-column label="操作" width="120">
                <template #default="scope">
                  ...
                  <el-button type="danger" plain link size="small" @click="handleConfigDelete([scope.row])">
                    <el-icon style="color:#f56c6c"><Delete /></el-icon> 删除
                  </el-button>
                </template>
              </el-table-column>
...
import { getUserList, addUser, updateUser, deleteUser, resetPassword, assignRoles, getUserRoleIds } from '../api/user';
import { getDeptTree, addDept, updateDept, deleteDept } from '../api/dept';
import { getRoleList, addRole, updateRole, deleteRole, getRoleUserIds, assignUsersToRole } from '../api/role';
import { Plus, Download, Refresh, Delete, Edit, Search } from '@element-plus/icons-vue';
...
async function handleUserDelete(rows) {
  const toDelete = rows || userSelection.value;
  if (!toDelete || !toDelete.length) {
    ElMessage.warning('请至少选择一项进行删除');
    return;
  }
  ElMessageBox.confirm(`确定要删除选中的 ${toDelete.length} 个用户吗？`, '提示', {
...
const roleDialog = reactive({
  visible: false,
  mode: 'add',
  form: { roleId: '', roleName: '', roleCode: '', status: 0, dataScope: 'ALL', deptIds: [], deptNames: [] },
  originForm: { roleId: '', roleName: '', roleCode: '', status: 0, dataScope: 'ALL', deptIds: [], deptNames: [] }
});
...
function showRoleDialog(mode, row) {
  ...
  if (mode === 'edit' && row) {
    Object.assign(roleDialog.form, {
      ...
      deptIds: row.deptIds || [],
      deptNames: (row.deptIds || []).map(id => getDeptNameById(id, deptTree.value))
    });
    roleDialog.originForm = { ...roleDialog.form };
  } else {
    Object.assign(roleDialog.form, { roleId: '', roleName: '', roleCode: '', status: 0, dataScope: 'ALL', deptIds: [], deptNames: [] });
    roleDialog.originForm = { roleId: '', roleName: '', roleCode: '', status: 0, dataScope: 'ALL', deptIds: [], deptNames: [] };
  }
...
async function handleRoleDialogOk() {
  roleFormRef.value.validate(async valid => {
    ...
    if (roleDialog.mode === 'add') {
      const { roleName, roleCode, status, dataScope, deptIds } = roleDialog.form;
      const submitData = { roleName, roleCode, status, dataScope, deptIds };
...
function clearRoleDialogForm() {
  ...
  roleDialog.form.dataScope = 'ALL';
  roleDialog.form.deptIds = [];
  roleDialog.form.deptNames = [];
  nextTick(() => roleFormRef.value && roleFormRef.value.clearValidate());
}
...
function handleUserToolbarEdit() {
  if (!userSelection.value || userSelection.value.length === 0) {
    ElMessageBox.alert('请选择要修改的用户。', '提示', { type: 'warning' });
    return;
  }
  if (userSelection.value.length > 1) {
    ElMessageBox.alert('只能选择一个用户进行编辑。', '提示', { type: 'warning' });
    return;
  }
...
// 在<script setup>中添加：
const customDeptDialog = reactive({
  visible: false,
  checkedKeys: []
});

function openCustomDeptDialog() {
  customDeptDialog.visible = true;
  // 回显已选
  customDeptDialog.checkedKeys = (roleDialog.form.deptIds || []).slice();
}

function handleCustomDeptOk() {
  roleDialog.form.deptIds = customDeptDialog.checkedKeys.slice();
  // 显示部门名称
  roleDialog.form.deptNames = (roleDialog.form.deptIds || []).map(id => getDeptNameById(id, deptTree.value));
  customDeptDialog.visible = false;
}

function handleCustomDeptCancel() {
  customDeptDialog.visible = false;
}

// 添加数据权限弹窗相关逻辑到<script setup>：
const dataScopeDialog = reactive({
  visible: false,
  form: { roleId: '', dataScope: 'ALL', deptIds: [], deptNames: [] }
});

function showDataScopeDialog(row) {
  dataScopeDialog.form.roleId = row.roleId;
  dataScopeDialog.form.dataScope = row.dataScope || 'ALL';
  dataScopeDialog.form.deptIds = row.deptIds ? row.deptIds.slice() : [];
  dataScopeDialog.form.deptNames = (row.deptIds || []).map(id => getDeptNameById(id, deptTree.value));
  dataScopeDialog.visible = true;
}

function handleDataScopeDialogOk() {
  const { roleId, dataScope, deptIds } = dataScopeDialog.form;
  updateRole({ roleId, dataScope, deptIds }).then(() => {
...
function openCustomDeptDialogDS() {
  customDeptDialog.visible = true;
  customDeptDialog.checkedKeys = dataScopeDialog.form.deptIds.slice();
  customDeptDialog._target = 'dataScopeDialog';
}

// 修改handleCustomDeptOk，支持dataScopeDialog弹窗：
if (customDeptDialog._target === 'dataScopeDialog') {
  dataScopeDialog.form.deptIds = customDeptDialog.checkedKeys.slice();
  dataScopeDialog.form.deptNames = (dataScopeDialog.form.deptIds || []).map(id => getDeptNameById(id, deptTree.value));
} else {
  roleDialog.form.deptIds = customDeptDialog.checkedKeys.slice();
  roleDialog.form.deptNames = (roleDialog.form.deptIds || []).map(id => getDeptNameById(id, deptTree.value));
}
