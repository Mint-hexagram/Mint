  ...
  <div class="defect-bg">
    ...
    <el-dialog v-model="editVisible" :title="editMode === 'add' ? '新增缺陷' : '编辑缺陷'" width="600px" @close="resetEditForm">
      <el-form :model="editForm" :rules="editRules" ref="editFormRef" label-width="120px">
        <el-form-item label="任务ID" prop="taskId">
          <el-input v-model.number="editForm.taskId" placeholder="请输入关联的任务ID" />
        </el-form-item>
        <el-form-item label="缺陷类型" prop="defectType">
        ...
        <el-form-item label="整改状态" prop="rectifyStatus">
          <el-select v-model="editForm.rectifyStatus" placeholder="请选择">
            <el-option label="未整改" :value="0" />
            <el-option label="整改中" :value="1" />
            <el-option label="已整改" :value="2" />
          </el-select>
    ...
    <el-dialog v-model="detailVisible" title="缺陷详情" width="600px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="建议整改方式">{{ selectedDefect.rectifyMethod }}</el-descriptions-item>
        <el-descriptions-item label="整改状态">
           <el-tag :type="getRectifyStatusType(selectedDefect.rectifyStatus)">{{ getRectifyStatusText(selectedDefect.rectifyStatus) }}</el-tag>
        </el-descriptions-item>
        <el-descriptions-item label="上报时间">{{ selectedDefect.reportTime }}</el-descriptions-item>
...
function handleQuickFilterChange(value) {
  ...
  if (value === 'today') {
    ...
    searchForm.rectifyStatus = 0; // 假设0是未整改
  }
