...
public class ExcelExportUtil {
    ...
    public static void exportTaskInfoList(List<TaskInfo> taskList, OutputStream os) throws IOException {
        ...
        Row header = sheet.createRow(0);
        String[] headers = {"任务ID", "任务编号", "任务名称", "任务类型", "优先级", "描述", "执行人ID", "协助人ID", "部门ID", "计划开始时间", "计划结束时间", "实际开始时间", "实际结束时间", "线路", "起始位置", "终止位置", "范围", "状态", "进度", "结果", "问题数", "创建时间"};
        for (int i = 0; i < headers.length; i++) {
            header.createCell(i).setCellValue(headers[i]);
        }
        DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        ...
        for (TaskInfo task : taskList) {
            ...
            row.createCell(17).setCellValue(task.getStatus() != null ? task.getStatus() : 0);
            row.createCell(18).setCellValue(task.getProgress() != null ? task.getProgress().doubleValue() : 0);
            row.createCell(19).setCellValue(task.getResult() != null ? task.getResult() : "");
            row.createCell(20).setCellValue(task.getProblemCount() != null ? task.getProblemCount() : 0);
            row.createCell(21).setCellValue(task.getCreateTime() != null ? task.getCreateTime().format(dtf) : "");
        }
        for (int i = 0; i < headers.length; i++) {
            sheet.autoSizeColumn(i);
        }
