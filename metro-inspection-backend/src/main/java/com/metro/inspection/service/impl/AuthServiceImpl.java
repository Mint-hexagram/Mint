...
public class AuthServiceImpl implements AuthService {
    ...
    public LoginResponse login(LoginRequest loginRequest) {
        ...
        // 生成JWT令牌 (使用原始角色名)
        String token = jwtUtils.generateToken(user.getUsername(), user.getUserId(), String.join(",", roles));

        // 将令牌存储到Redis，设置过期时间
        redisTemplate.opsForValue().set("token:" + token, user.getUsername(), 24, TimeUnit.HOURS);

        return new LoginResponse(token, user.getUserId(), user.getUsername(), user.getNickname(), roles, List.of());
    }
    ...
    public String refreshToken(String token) {
        ...
        // 生成新令牌
        String newToken = jwtUtils.generateToken(user.getUsername(), user.getUserId(), String.join(",", roles));
